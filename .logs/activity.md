...
## BRAINSTORM — Sprint 7 (2026-07-17, session 7)
Presented 4 scope decisions to user (email delivery, dispute-release event firing, notification type modeling, analytics computation):
- Email delivery: user's first answer was their own email address (rrhorba.mohamed@gmail.com) instead of picking an option - clarified with a follow-up question rather than guessing. Real Gmail SMTP was offered as the literal reading but user picked mock/log-only, no SMTP at all - matches the CMI_MODE=mock precedent (Story 7.3 revisits real CMI creds; same deferral pattern now applies to email).
- Dispute-resolution escrow release (AdminBookingDisputeService.resolve, RELEASE outcome): will also fire BookingCompletedEvent/EscrowReleasedEvent, not just its existing DISPUTE_RESOLVED notification - user picked consistency across both completion paths.
- Notification.type: stays a raw String (no enum) - YAGNI, user picked no unnecessary churn across the 7 existing call sites.
- Admin analytics (6.3): real-time aggregate queries on dashboard load, no precomputed metrics table - YAGNI at current data scale.

## PLAN — Sprint 7 (2026-07-17, session 7)
User-approved 3-batch plan:
- Batch 1 (Story 6.1): @EnableAsync; BookingCompletedEvent/EscrowReleasedEvent/TutorVerifiedEvent/ReviewSubmittedEvent; NotificationEventListener (@EventListener @Async, creates notification row + sends email per event); EmailProvider interface + MockEmailProvider (mirrors MockEscrowPaymentProvider's plain @Component pattern, no conditional bean selection since no real impl exists yet); EMAIL_MODE=mock added to .env.example for a future real-SMTP story. Publish events at: BookingCompletionService.confirmCompletion (both events, replacing its 2 direct BOOKING_COMPLETED calls), AdminBookingDisputeService.resolve RELEASE branch (both events, additive to existing DISPUTE_RESOLVED), AdminVerificationService.approve (TutorVerifiedEvent), ReviewService.submit (ReviewSubmittedEvent, replacing its direct REVIEW_RECEIVED call). The 5 other existing notification types stay as direct synchronous calls, unchanged.
- Batch 2 (Story 6.2): NotificationRepository query methods + NotificationService list/markRead + NotificationController (GET list, PATCH mark-read); Angular bell icon + notification panel, FR/AR/EN i18n.
- Batch 3 (Story 6.3): AdminAnalyticsController+service, real-time aggregate queries (signups, bookings count, GMV = sum of completed booking prices, match rate = gigs with >=1 match / total gigs); Angular admin dashboard page.
- Verify/Ship: standard (coverage >=80% both, manual secret/IDOR review, push, CI monitor, log).

## EXECUTE — Sprint 7 Batch 1 (2026-07-17, session 7)
Story 6.1 (domain-event-driven notifications + email) shipped to working tree:
- New notification.event package: BookingCompletedEvent/EscrowReleasedEvent/TutorVerifiedEvent/ReviewSubmittedEvent (records carrying recipient + entity IDs). BookingCompletedEvent/EscrowReleasedEvent are published once per trigger carrying both studentUserId/tutorUserId, not twice per recipient - listener fans out to both parties internally.
- notification.email.EmailProvider interface + MockEmailProvider (mirrors MockEscrowPaymentProvider's plain @Component pattern exactly, logs the email). EMAIL_MODE=mock added to .env.example (and local .env, gitignored) alongside the existing CMI_MODE precedent - not wired to any conditional yet, forward-declared for a future real-SMTP story.
- NotificationEventListener (@Async @EventListener x4) creates the in-app notification row (existing NotificationService.create) and sends email via EmailProvider for each of the 4 events. Notification type strings: BOOKING_COMPLETED, ESCROW_RELEASED (new), TUTOR_VERIFIED (new), REVIEW_RECEIVED (unchanged string).
- @EnableAsync added to BackendApplication.
- Publishers wired at all 4 trigger points per plan: BookingCompletionService.confirmCompletion (replaced 2 direct BOOKING_COMPLETED calls), AdminBookingDisputeService.resolve RELEASE branch (additive to existing DISPUTE_RESOLVED calls, confirmed still present), AdminVerificationService.approve (additive to existing VERIFICATION_APPROVED call), ReviewService.submit (replaced direct REVIEW_RECEIVED call).
- Tests: new NotificationEventListenerTest (5 cases incl. missing-user-skips-email edge case); BookingCompletionServiceTest/AdminBookingDisputeServiceTest/AdminVerificationServiceTest/ReviewServiceTest updated to assert publishEvent(...) instead of direct notification calls.
Full `mvn verify` (run independently by the orchestrator after the executing subagent's own run had reporting trouble): BUILD SUCCESS, 94 unit + 55 integration tests green, 92.55% instructions / 87.30% branches - clears the 80% gate. Diff spot-checked: DISPUTE_RESOLVED calls intact, .env confirmed gitignored (not leaking EMAIL_MODE/secrets), no unrelated changes.
Next: Batch 2 (Story 6.2, notification center UI).

## EXECUTE — Sprint 7 Batch 2 (2026-07-17, session 7)
Story 6.2 (notification center UI) shipped to working tree, implemented directly (not delegated to a subagent, given Batch 1's forking friction):
- Backend: Notification entity gained a markRead() mutator; NotificationRepository.findByUserIdOrderByCreatedAtDesc; NotificationService.listForUser/markRead (ownership-checked, 403 for non-owner, 404 for missing); NotificationResponse DTO; NotificationController (GET /api/v1/notifications, PATCH /api/v1/notifications/:id/read) - falls under the existing anyRequest().authenticated() security rule, no new matcher needed. Tests: NotificationServiceTest (5 cases), NotificationControllerIT (3 cases: list+mark-read happy path via a real BOOKING_CREATED notification, IDOR 403 on mark-read, empty-list-before-any-notification).
- Frontend: core/notification/{notification.models,notification.service,notification.service.spec}.ts (GET/PATCH per backend routes); shared/notification-bell component (mat-icon-button + badge + mat-menu dropdown, fetch-on-first-open not on every render, click-to-mark-read) wired into app.html/app.ts's toolbar next to the language menu, visible only when authenticated. FR/AR/EN i18n added for all 9 notification type strings (including the 5 non-event ones from Sprint 1-6) plus title/empty strings.
- Discovered mid-batch: this frontend project uses Vitest, not Karma/Jasmine - first spec draft used jasmine.createSpyObj/TranslateModule.forRoot() and failed to compile; corrected to vi.fn()/provideTranslateService() matching booking-detail.component.spec.ts's established pattern.
Backend: ran independently via `./mvnw -q -Dtest=NotificationServiceTest,NotificationControllerIT test` - EXIT_CODE=0, all passing (full-suite mvn verify deferred to the end-of-sprint VERIFY phase to avoid redundant full runs mid-batch).
Frontend: `npm run test -- --watch=false` (CI's exact command) - 26 test files / 103 tests passed, EXIT_CODE=0.
Next: Batch 3 (Story 6.3, admin analytics dashboard).

## VERIFY (partial) — Sprint 7 Batch 2 coverage (2026-07-17, session 7)
`ng test --coverage`: 26 test files / 103 tests green, 86.32% statements / 90.54% branches / 78.74% functions / 91.94% lines - clears the 80% gate. Full sprint-level VERIFY (backend+frontend combined, security review) still pending after Batch 3.

## EXECUTE — Sprint 7 Batch 3 (2026-07-17, session 7)
Story 6.3 (admin analytics dashboard) shipped to working tree:
- Backend: UserRepository.countByRole; BookingRepository.countByStatus + sumAgreedPriceMadByStatus (native-ish JPQL with coalesce to avoid null on zero rows); MatchSuggestionRepository.countDistinctGigRequestId (match rate numerator). AdminAnalyticsService.compute() aggregates: studentSignups, tutorSignups, totalBookings, completedBookings, gmvMad (sum of COMPLETED bookings' agreedPriceMad), matchRatePercent (matched gigs / total gigs * 100, 0 when no gigs yet - guarded against division by zero). AdminAnalyticsController: GET /api/v1/admin/analytics, falls under existing hasRole("ADMIN") matcher. Tests: AdminAnalyticsServiceTest (2 cases incl. zero-gigs edge case), AdminAnalyticsControllerIT (3 cases: signup/booking-count deltas, GMV+completed-count delta after a real mutual-completion flow, non-admin 403) - used before/after delta assertions rather than absolute counts since the shared Testcontainers Postgres instance persists data across test methods within a class.
- Frontend: core/admin/admin-analytics.service.ts (+ spec), AdminAnalytics added to admin.models.ts; features/admin/analytics-dashboard component (4 metric cards: signups w/ student+tutor breakdown, bookings w/ completed count, GMV, match rate) wired to new /admin/analytics route (roleGuard ADMIN, no nav link added - matches existing precedent that admin/verification and admin/disputes also have no in-app nav link, reached by direct URL). FR/AR/EN i18n added under the existing admin.analytics namespace.
- Caught and fixed own mistake mid-batch: first en.json edit added "admin.analytics" as a literal top-level key (dotted string) instead of nesting analytics inside the existing admin object - ngx-translate resolves dotted keys via object nesting, so this would have silently failed to resolve. Corrected in en.json before running tests; caught and fixed a resulting duplicate "notifications" block in fr.json from the same copy-paste (created via old_string matching the pre-fix file ending).
Backend: `./mvnw -q -Dtest=AdminAnalyticsServiceTest,AdminAnalyticsControllerIT test` - EXIT_CODE=0, all passing (also re-confirms Batch 1's event/email wiring end-to-end via the mutual-completion delta test's MockEmailProvider log lines).
Frontend: `ng test --coverage` - 28 test files / 107 tests green, 86.83% statements / 91.01% branches / 79.53% functions / 92.24% lines - clears the 80% gate.
Next: sprint-level VERIFY (full mvn verify, full security/IDOR review) then SHIP.

## VERIFY — Sprint 7 (2026-07-17, session 7)
Full-sprint verification across all 3 batches:
- Backend: `./mvnw -q verify` from a clean run - BUILD SUCCESS, all unit + integration tests green (including the new NotificationEventListenerTest, NotificationServiceTest, NotificationControllerIT, AdminAnalyticsServiceTest, AdminAnalyticsControllerIT, plus updated BookingCompletionServiceTest/AdminBookingDisputeServiceTest/AdminVerificationServiceTest/ReviewServiceTest). Coverage: 93% instructions / 87% branches - clears the 80% gate.
- Frontend: `ng test --coverage` - 28 test files / 107 tests green, 86.83% statements / 92.24% lines - clears the 80% gate.
- Security review: no local gitleaks binary (consistent with sessions 3-6); manual diff review across all ~30 changed/new files found no secrets - .env.example only documents EMAIL_MODE=mock (no key/value), .env itself confirmed gitignored. No string-concatenated SQL anywhere in the diff (grep-verified) - all new queries are JPQL with bound params. IDOR: NotificationController.markRead checks ownership (403 for non-owner, tested); notification list is scoped to the authenticated user's own rows only (no id-based lookup endpoint exists for other users' notifications). AdminAnalyticsController/AdminAnalyticsService expose only aggregate counts/sums, no per-user or per-booking PII, and sit under the existing hasRole("ADMIN") matcher. Event payloads (BookingCompletedEvent etc.) derive recipient user IDs server-side from the booking/document record, never from client input, closing the same tampering vector as prior sprints' reviews/messaging work.
...
## PLAN — Sprint 8 (2026-07-18, session 8)
Batch 1 (Story 7.1, i18n/RTL pass): audit all screens for hardcoded strings vs translation usage across every feature area (auth, profile, gigs, matching, bookings, reviews, messaging, notifications, admin); fill fr/ar/en translation key gaps; RTL smoke-check every screen; add i18n test coverage for new keys.
Batch 2 (Story 7.2, security confirmation pass): trace every Test Strategy §4 checklist line to an existing test; re-run full suites + CI scans; write new tests only for confirmed gaps.
Batch 3 (Story 7.4, E2E + video + prod deploy): add Playwright (new tooling, not previously set up) for browser E2E; write the critical-path spec (registration→verification→gig→match→booking→escrow→completion→review) run in FR and AR; add docker-compose.prod.yml (COOKIE_SECURE=true, prod-safe overrides - Dockerfiles already build production artifacts, no changes needed there); run the E2E suite with video recording against the prod-mode stack, save to .recordings/v1-2026-07-18.webm; check release gate criteria (Test Strategy §5).
Then Sprint 8 VERIFY (coverage ≥80%, security check) and SHIP (push, milestone log, video log).

## EXECUTE — Sprint 8 Batch 1 complete: Story 7.1 i18n/RTL full translation pass (2026-07-19, session 8)
Static audit: all 3 locale files (fr/en/ar) have identical 142-key sets (zero drift), zero hardcoded user-facing strings found across all 14 templates/TS files, zero physical-direction CSS (no margin-left/right etc. to break under RTL) - RTL is centrally handled via I18nService setting document.documentElement.dir, already unit-tested.
Live browser verification (Docker Desktop + backend on :8082 + frontend on :4300, local env - ports adjusted due to another local project occupying 5432/8080): found and fixed a CRITICAL bug (see .logs/issues.md "Translations never loaded at runtime") - provider order in app.config.ts caused the real HTTP translation loader to be silently shadowed by ngx-translate's default no-op loader on every bootstrap. Fixed by reordering provideTranslateService before provideTranslateHttpLoader.
Post-fix, verified full FR/EN/AR translation + correct RTL mirroring (toolbar, form alignment, radio button order, empty states) live on: login, register, gig-create-form, tutor-browse (incl. empty state), notification-bell (empty state), tutor-profile-form, and all 3 admin screens (verification-queue, dispute-queue, analytics-dashboard empty/zero states) - covers every route in app.routes.ts except gig-detail/booking-confirm/booking-detail/tutor-profile-view (not exercised live this session due to needing seeded gig/booking data; already covered by the static audit which found no gaps, and share the same template/translate patterns validated everywhere else).
Test accounts created for verification (sara.student@example.com, tara.tutor@example.com, one promoted to ADMIN via direct DB update) were deleted after use - no test data left in the dev DB.

## EXECUTE — Sprint 8 Batch 2 complete: expired/tampered JWT test + full-suite regression fix (2026-07-20, session 9)
Added JwtAuthFilterIT (expired token, tampered signature, missing token - all assert 403, matching this codebase's established convention of AccessDeniedException over a custom 401 entry point, confirmed by grepping every other ControllerIT's unauthenticated-access assertions). Tampering test flips the FIRST base64url char of the signature segment, not the last - the last char of a 32-byte HMAC-SHA256 signature's base64url encoding carries 2 unused padding bits that java.util.Base64's URL decoder (used by JJWT) doesn't validate, so a first attempt at flipping the last char silently decoded to the same signature bytes and the test false-passed at 200 instead of catching a real rejection.
Running the full suite (not just the new test) surfaced a real regression from the Batch 2 content-sniffing fix (commit 245093b, previous session): two pre-existing tests (AdminVerificationControllerIT x5 assertions via a shared fixture, VerificationDocumentControllerIT.tutor_canUploadValidDiploma) used placeholder "content"/"diploma-bytes" byte arrays without real PDF magic bytes, so they started failing the new content-sniffing check with 400 instead of 201. Fixed both fixtures to use "%PDF-1.4 ..." bytes, and fixed one downstream assertion (admin_canFetchDecryptedDocumentContent) that asserted the exact decrypted byte content. This is exactly why Batch 2's own EXECUTE note flagged "only ran the narrow unit test" as a gap - the full-suite VERIFY step below is what caught it.
Next: Batch 2 VERIFY (see below), then Batch 3 (Playwright E2E + prod compose + video).

## VERIFY — Sprint 8 Batch 2 (2026-07-20, session 9)
Backend: `./mvnw -q verify` - BUILD SUCCESS, all tests green (including new JwtAuthFilterIT and the two fixed ITs). Coverage: 93.15% instructions / 85.92% branches (JaCoCo) - clears the 80% gate.
Frontend: `ng test --coverage --watch=false` - first run had 1 failure (tutor-profile-form "loads and populates the existing profile on init", timed out at 5000ms). This is the same flake first seen in session 5 (then attributed to "load"); it has now recurred a second time under a different workload, so it's a genuine intermittent flake, not a one-off. Re-ran standalone: 8/8 pass. Re-ran the full suite fresh: 28/28 files, 107/107 tests pass, 86.83% statements / 92.24% lines - clears the 80% gate. See .logs/issues.md for a flagged follow-up on de-flaking it.
Security: no local gitleaks binary (consistent with every prior session); manual diff review of all 4 changed/new test files (JwtAuthFilterIT.java, AdminVerificationControllerIT.java, VerificationDocumentControllerIT.java, plus the already-reviewed Batch 1/2 source changes from commit 245093b) found no secrets - the JWT test's signing key constant intentionally mirrors application.yml's existing dev-only fallback string, not a new secret. Test Strategy §4 checklist is now fully traced: the JWT expired/tampered/missing-token gap identified at the end of session 8 is closed.
Sprint 8 Batch 2 (Story 7.2) DoD met. Next: Batch 3 (Story 7.4 - Playwright E2E, docker-compose.prod.yml, video recording, release gate check).

## EXECUTE — Sprint 8 Batch 3, part 1: docker-compose.prod.yml, Playwright scaffold, 5 stacked deployment bugs (2026-07-20, session 9)
docker-compose.prod.yml added as an override (COOKIE_SECURE=true, SPRING_PROFILES_ACTIVE=prod, restart: unless-stopped on all 3 services) - Dockerfiles already build production artifacts per the PLAN, no separate prod Dockerfiles needed.
Playwright scaffolded at e2e/ (separate npm project, not inside frontend/'s Vitest setup): playwright.config.ts (baseURL configurable via E2E_BASE_URL, video on-demand via E2E_RECORD_VIDEO), tests/critical-path.spec.ts covering registration -> tutor verification (real magic-byte-valid PDF upload) -> admin approval (via a throwaway account promoted to ADMIN through a direct DB UPDATE, mirroring session 8's manual-verification precedent since no admin-registration API exists by design) -> gig creation -> booking (direct URL to the known verified tutor rather than waiting on a live match card, logged in decisions.md) -> mutual completion -> review, parameterized to run once each in FR and AR (also asserts `dir="rtl"` on the AR run). Chromium installed via `npx playwright install chromium`.

Bringing up docker-compose.prod.yml for the first time to actually run the E2E suite surfaced that the backend image had never successfully started as a packaged Docker image before - 5 stacked bugs in the embedding provider, fixed in sequence (full detail per-bug in .logs/issues.md "Backend crash-loops on startup: 4 stacked deployment bugs..."):
1. Wrong DJL model-zoo group id (`ai.djl.huggingface.onnxruntime` -> `ai.djl.huggingface.pytorch`).
2. djl-cache Docker volume created root-owned, unwritable by the non-root `app` user - chowned it in the Dockerfile before USER app.
3. Alpine (musl) base image incompatible with ONNX Runtime's glibc-linked native library - switched backend/Dockerfile's runtime stage from `eclipse-temurin:25-jre-alpine` to `eclipse-temurin:25-jre` (Ubuntu), updating user-creation commands (addgroup/adduser -> groupadd/useradd) and adding apt-installed wget for the existing HEALTHCHECK.
4. HuggingFace tokenizer JNI library had no writable $HOME (useradd -r doesn't create one) - added ENV HOME=/app.
5. DJL's curated HF zoo only serves this model's PyTorch weights (not ONNX), and DJL's own direct-URL downloader loses the .onnx extension through HuggingFace's CDN redirect - rewrote LocalMultilingualEmbeddingProvider to download the real ONNX export (confirmed live at .../resolve/main/onnx/model.onnx) itself via java.net.http.HttpClient into an explicit local path, and build the HuggingFace tokenizer + mean-pooling/normalize TextEmbeddingTranslator explicitly (bypassing the zoo's translator wiring along with its model resolution).
